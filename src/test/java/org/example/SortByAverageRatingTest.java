...
class SortByAverageRatingTest {
    ...
    @Test
    void compare_SecondArgumentIsSame() {
        sortByAverageRating = new SortByAverageRating();
        Student one = createStudent1();
        Student two = createStudent2();
        one.setAverageRating(5);
        two.setAverageRating(5);
        int expectedRezult = one.getAverageRating() - two.getAverageRating();
        int actualRezult = sortByAverageRating.compare(one, two);
        assertEquals(expectedRezult, actualRezult);
    }
}
